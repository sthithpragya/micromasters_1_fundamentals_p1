function [ ik_sol ] = puma_ik_9( x, y, z, R )
...
%    the desired end effector position and orientation.
    ...
    if pwc(1)^2 + pwc(2)^2 + (pwc(3)-13)^2 < 25 || pwc(1)^2 + pwc(2)^2 + (pwc(3)-13)^2 > 281
        ...
        X = (pwc(3)-13)/8;
        if sin(theta1(1)) == 0
            Y1 = pwc(1)/(8*cos(theta1(1)));
        else
            Y1 = (pwc(1) - 5*cos(theta1(1)))/(8*sin(theta1(1)));
        end
        
        if sin(theta1(2)) == 0
            Y2 = pwc(1)/(8*cos(theta1(2)));
        else
            Y2 = (pwc(1) - 5*cos(theta1(2)))/(8*sin(theta1(2)));
        end
