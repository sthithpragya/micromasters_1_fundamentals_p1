...
%    the desired end effector position and orientation.
    ...
    if pwc(1)^2 + pwc(2)^2 + (pwc(3)-13)^2 < 25 || pwc(1)^2 + pwc(2)^2 + (pwc(3)-13)^2 > 281
        ...
        X = (pwc(3)-13)/8;
        if cos(theta1(1)) == 0
            Y = pwc(2)/(8*sin(theta1(1)));
        else
            Y = (pwc(1) + 5*sin(theta1(1)))/(8*cos(theta1(1)));
        end
        t2minus3 = [asin((X^2+Y^2-2)/2) pi-asin((X^2+Y^2-2)/2)];
        K = sin(-t2minus3(1));
        Z = (X^2 - Y^2)/(2*sqrt(1+K^2));
        G = atan2(K,1);
        t3plus2 = [asin(Z)-G pi-asin(Z)-G]  ;
        theta2 = [(t3plus2(1) + t2minus3(1))/2 (t3plus2(1) + t2minus3(2))/2 (t3plus2(2) + t2minus3(1))/2 (t3plus2(2) + t2minus3(2))/2];
        theta3 = [(t3plus2(1) - t2minus3(1))/2 (t3plus2(1) - t2minus3(2))/2 (t3plus2(2) - t2minus3(1))/2 (t3plus2(2) - t2minus3(2))/2];
        ...
        for i1 = 1:2
            for i2 = 1:4
                for i3 = 1:4          
                    pwc11 = [pwc11; -5*sin(theta1(i1)) + 8*cos(theta1(i1))*(cos(theta2(i2)) - sin(theta3(i3)))]; %%
                    if -5*sin(theta1(i1)) + 8*cos(theta1(i1))*(cos(theta2(i2)) - sin(theta3(i3))) == pwc(1)
                            pwc1 = [pwc1; [i1 i2 i3]];
                    else
                            pwc1 = [pwc1; []];
                    end
        ...
        for i1 = 1:2
            for i2 = 1:4
                for i3 = 1:4 
                    pwc22 = [pwc22; 5*cos(theta1(i1)) + 8*sin(theta1(i1))*(cos(theta2(i2)) - sin(theta3(i3)))]; %%
                    if 5*cos(theta1(i1)) + 8*sin(theta1(i1))*(cos(theta2(i2)) - sin(theta3(i3))) == pwc(2)
                            pwc2 = [pwc2; [i1 i2 i3]];
                    else
                            pwc2 = [pwc2; []];
                    end
        ...
        for i1 = 1:2
            for i2 = 1:4
                for i3 = 1:4          
                    pwc33 = [pwc33; 13 + 8*(sin(theta2(i2)) + cos(theta3(i3)))]; %%
                    if 13 + 8*(sin(theta2(i2)) + cos(theta3(i3))) == pwc(3)
                            pwc3 = [pwc3; [i1 i2 i3]];
                    else
                            pwc3 = [pwc3; []];
                    end
