function [ ik_sol ] = puma_ik_20_round( x, y, z, R )
...
%    the desired end effector position and orientation.
    ...
    if pwc(1)^2 + pwc(2)^2 + (pwc(3)-13)^2 < 25 || pwc(1)^2 + pwc(2)^2 + (pwc(3)-13)^2 > 281
        ...
        theta2 = [asin(round(((pwc(3)-13)/8))); pi-asin(round(((pwc(3)-13)/8)))];        
        theta1 = [asin(round(((8/5)*pwc(2)*cos(theta2(1))-pwc(1))/((64/5)*cos(theta2(1))*cos(theta2(1))+5))), asin(round(((8/5)*pwc(2)*cos(theta2(2))-pwc(1))/((64/5)*cos(theta2(2))*cos(theta2(2))+5)));
            pi-asin(round(((8/5)*pwc(2)*cos(theta2(1))-pwc(1))/((64/5)*cos(theta2(1))*cos(theta2(1))+5))), pi-asin(round(((8/5)*pwc(2)*cos(theta2(2))-pwc(1))/((64/5)*cos(theta2(2))*cos(theta2(2))+5)))];
        ...
        for i2 = 1:2
            for i1 = 1:2
                for i3 = 1:2
                    ...
                    T01 = compute_dh_matrix(0, pi/2, 13, theta1(i1,i2));
                    T12 = compute_dh_matrix(8, 0, -2.5, theta2(i2));
                    ...
                    if round(double(T04(1:3,4))) == round(pwc2)
                            theta_checker = [theta_checker; [i2 i1 i3]];
                    end
                end
            end
        end
        THETA1 = theta1(theta_checker(1,2),theta_checker(1,1));
        THETA2 = theta2(theta_checker(1,1));
        THETA3 = theta3(theta_checker(1,3));
        ...
        if R36_ik(3,3) == 1
            ...
            theta5 = [acos(R36_ik(3,3)), -acos(R36_ik(3,3))];
            theta6 = zeros(1,2);
            theta4 = zeros(1,2);
            for i = 1:2
                if sin(theta5(i)) > 0
                    theta6(i) = atan2(-R36_ik(3,2),R36_ik(3,1));
                    theta4(i) = atan2(-R36_ik(2,3),-R36_ik(1,3));
                else
                    theta6(i) = atan2(R36_ik(3,2),-R36_ik(3,1));
                    theta4(i) = atan2(R36_ik(2,3),R36_ik(1,3));
                end
            ...
            for i = 1:2
                t34 = compute_dh_matrix(0, pi/2, 8, theta4(i));
                t45 = compute_dh_matrix(0, -pi/2, 0, theta5(i));
                t56 = compute_dh_matrix(0, 0, 2.5, theta6(i));
                t36 = t34*t45*t56;
                R36 = t36(1:3,1:3);
                if double(round(R36)) == double(round(R36_ik))
                    THETA4 = theta4(i);
                    THETA5 = theta5(i);
                    THETA6 = theta6(i);
                end
