...
while true
    ...
    [min_Hvalue, Index_value] = min(Hvalues);
    next_index = Index(Index_value);
    if min_Hvalue ~= inf
        map(next_index) = 4;
        parent(next_index) = current;
        g(next_index) = g(current) + 1;
        f(next_index) = min_f + 1;
        numExpandedcheck(current) = 1;
        numExpanded = numExpanded + 1;
        
    else
        if current ~= start_node
            if numExpandedcheck(current) == 1
                numExpandedcheck(current) = 0;
                numExpanded = numExpanded + 1;
            end
